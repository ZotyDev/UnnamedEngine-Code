...
/// Generates a random 64 bit integer that is exactly `n` characters long when
/// encoded as base62.
///
...
/// # Panics
///
/// This method panics if `n` is 0 or greater than 11, since a `u64` can only
/// represent up to 11 character base62 strings.
#[inline]
...
/// Generates a random 64 bit integer that is exactly `n` characters long when
/// encoded as base62, using the given rng.
///
/// # Panics
///
/// This method panics if `n` is 0 or greater than 11, since a `u64` can only
...
pub fn random_base62_rng<R: rand::RngCore>(rng: &mut R, n: usize) -> u64 {
    random_base62_range(rng, n, n)
}

pub fn random_base62_range<R: rand::RngCore>(
    rng: &mut R,
    n_min: usize,
    n_max: usize,
) -> u64 {
    ...
    // gen_range is [low, high]; max value is `MULTIPLES[n] - 1`, which is n
    // characters long when encoded
...
/// An ID encoded as base62 for use in this API.
///
/// All ids should be random and encode to 8-10 character base62 strings, to
/// avoid enumeration and other attacks.
...
pub mod base62_impl {
    ...
    pub fn to_base62(mut num: u64) -> String {
        ...
        while num > 0 {
            // Could be done more efficiently, but requires byte manipulation of
            // strings & Vec<u8> -> String conversion
