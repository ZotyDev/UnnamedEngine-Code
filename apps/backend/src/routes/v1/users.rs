use axum::{
    http::StatusCode, response::IntoResponse, routing::get, Extension, Json,
    Router,
};
...
pub fn routes() -> Router {
    Router::new().route("/status", get(status_get))
}
...
async fn status_get(
    ...
    let results =
        crate::database::models::user_item::Status::list(&database, &redis)
            .await
