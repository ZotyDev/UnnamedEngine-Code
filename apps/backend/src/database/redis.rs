...
impl RedisPool {
    /// Initiates a new redis pool.
    ///
    /// # Panics
    ///
    /// This method panics if `REDIS_URL` is not set.
    pub fn new(meta_namespace: Option<String>) -> Self {
        let redis_pool = deadpool_redis::Config::from_url(
            dotenvy::var("REDIS_URL").expect("`REDIS_URL` not set"),
        )
        .builder()
    ...
    where
        ...
        T: Serialize + DeserializeOwned,
        I: Debug + Display + Hash + Eq + PartialEq + Clone,
        K: Debug
            + Display
            + Hash
            + Eq
        ...
        let get_cached_values =
            |ids: DashMap<String, I>,
             mut connection: deadpool_redis::Connection| async move {
                ...
                let cached_values = redis::cmd("MGET")
                    .arg(
                        ids.iter()
...
pub fn redis_args(cmd: &mut redis::Cmd, args: &[String]) {
    for arg in args {
        cmd.arg(arg);
    }
}
